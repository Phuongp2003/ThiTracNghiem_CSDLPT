...
<table class="table bode-table">
	<thead>
		<tr>
			...
			<c:choose>
				<c:when test="${role_al == 'TRUONG'}"><th style="width: 12%">Giáo viên</th></c:when>
				<c:otherwise><th style="width: 12%">Thao tác</th></c:otherwise>
			</c:choose>
		</tr>
	</thead>
	<tbody>
		<c:forEach var="bd" items="${bodes}">
			<tr>
                ...
                <td>${bd.DAP_AN}</td>
				<c:choose>
					<c:when test="${role_al == 'TRUONG'}"><td>${bd.MAGV}</td></c:when>
					<c:otherwise>
						<td>

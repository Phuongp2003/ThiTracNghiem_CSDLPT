	...
	<tbody>
		<c:forEach var="k" items="${khoas}">
			<tr class="is-action sl-${k.MAKH}">
				...
				<c:if test="${role_al != 'TRUONG' && role_al != 'GIANGVIEN'}">
					<td>
						<form class="d-inline" action="department-class/delete-department/${k.MAKH.trim()}.htm" method="post">
							<button type="button" onclick="if (confirm('Bạn có chắc muốn xóa khoa ${k.TENKH}?')) submitClosestForm(this, () => refreshData())" class="btn btn-outline-primary" data-bs-dismiss="modal"><i class="bi bi-trash3-fill"></i></button>
						</form>
